...
export class GetSalidaUseCase {
  ...
  async getSalidasPorMes(mes: number, anio: number, categoriaId: number, page = 1, limit = 0): Promise<{data: Array<{salida_id: number; fecha_salida: string; cantidad: number; codigo_producto: string; nombre: string;}>; total: number;}> {
