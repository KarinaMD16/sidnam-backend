...
export class GetEntradaUseCase {
  ...
  async getEntradasPorMes(
    ...
    const qb = this.entradaRepository
      ...
      .addSelect("DATE_FORMAT(e.fechaEntrada, '%Y-%m-%d')", 'fecha_entrada') // MySQL; en PG usa TO_CHAR(...)
    ...
    // ✅ usa offset/limit (más confiable que skip/take con raw)
    if (limit > 0) {
